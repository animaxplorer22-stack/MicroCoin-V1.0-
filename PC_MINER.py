#!/usr/bin/env python3
"""
MICROCORE (MCX) PC MINER v5.0 - FULL VERSION
...
class MinerStats:
    ...
    def save_stat(self, key: str, value: int):
        c = self.conn.cursor()
        c.execute("INSERT OR REPLACE INTO miner_stats VALUES (?, ?, ?)", (key, value, time.time()))
        self.conn.commit()
    ...
    def record_block(self, block_id: int, reward: int, node: str, challenge: str):
        c = self.conn.cursor()
        c.execute("INSERT INTO blocks_mined VALUES (?, ?, ?, ?, ?)", (block_id, time.time(), reward, node, challenge[:32]))
        self.conn.commit()
    
    def record_miss(self, block_id: int, reason: str = "Timeout"):
        c = self.conn.cursor()
        c.execute("INSERT INTO missed_blocks VALUES (?, ?, ?)", (block_id, time.time(), reason))
        self.conn.commit()
    
    def record_node_switch(self, from_node: str, to_node: str):
        c = self.conn.cursor()
        c.execute("INSERT INTO node_switches (timestamp, from_node, to_node) VALUES (?, ?, ?)", (time.time(), from_node, to_node))
        self.conn.commit()
    
    def record_slash(self, amount: int, reason: str):
        c = self.conn.cursor()
        c.execute("INSERT INTO slash_events (timestamp, amount, reason) VALUES (?, ?, ?)", (time.time(), amount, reason))
        self.conn.commit()
    
    def record_daily_uptime(self, date: str, seconds: int):
        c = self.conn.cursor()
        c.execute("INSERT OR REPLACE INTO daily_uptime VALUES (?, ?)", (date, seconds))
        self.conn.commit()
...
class PCMiner:
    ...
    def load_stats(self):
        """Load saved stats from database"""
    ...
    def add_log(self, msg: str, msg_type: str = "info"):
        ...
        if msg_type == "success":
            ...
            color = "\033[94m"
        print(f"[{timestamp}] {color}{msg}\033[0m")
    
    def get_current_peer_url(self) -> Optional[str]:
        """Get current peer URL from peer list"""
        if not self.peers:
            return None
    ...
    def add_peer_from_gossip(self, peer: str):
        """Add new peer discovered via gossip"""
    ...
    def switch_to_next_peer(self):
        """Switch to next peer in list for failover"""
    ...
    def add_reward(self, reward: int, block_id: int = 0, challenge: str = ""):
        """Add block reward to miner's stake"""
        ...
        self.current_level = self.calculate_level()
        self.save_stats()
        self.stats_db.record_block(block_id, reward, self.get_current_peer_url() or "unknown", challenge)
...
# ==================== WEBSOCKET COMMUNICATION ====================
    async def register(self):
        ...
        if self.websocket:
            ...
            self.add_log(f"[REG] Registered as '{self.wallet.username}'", "info")
    ...
    async def sign_block(self):
        """Sign the current block challenge"""
        ...
        if self.websocket:
            await self.websocket.send(json.dumps(msg))
            self.add_log(f"[SIGN] Signed block {self.current_block_id}", "success")
    
    async def send_status(self):
        """Send miner status to node"""
        msg = {
            "type": "miner_status",
            "validator_id": self.validator_id,
            "username": self.wallet.username,
    ...
    async def handle_message(self, data: str):
        ...
        try:
            ...
            # Gossip discovery - receive peers
            elif msg_type == "peers":
                ...
                async with websockets.connect(
                    ...
                    while self.running and self.mining_enabled and self.connected:
                        ...
                        if self.is_validator and (time.time() - self.last_challenge_time) > (SIGNING_WINDOW_MS / 1000 + 0.5):
                            ...
                            self.handle_slash(0, "Fallback timeout")
                            self.is_validator = False
...
async def main():
    print("\n" + "=" * 60)
    print("🔷 MICROCORE PC MINER - FULL VERSION 🔷")
    print("=" * 60)
